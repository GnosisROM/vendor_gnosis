...
if [ -z ${source_tree} ]; then
		...
		repo forall -pc 'git log --oneline --after='$1' --until='$2 | grep -v "Automatic translation import" | grep -v "Merge" | sed 's/^$/#EL /' | sed 's/^/* /' | sed 's/* #EL //' | sed 's/* //' | while read string; do
			...
			if [ -n "${string##+([:space:])}" ]; then
				if [ "$project" == 0 ]; then
					...
					temp_two=$(echo "$string" | sed 's/^[^ ]* //g')
					echo "* [$temp_one](https://github.com/search?q=$( rawurlencode "$temp_one" )&type=Commits)""* [$temp_two](https://android.googlesource.com/search?q=$( rawurlencode "$temp_two" )&type=Commits)" >> $Changelog;
				fi;
			fi
		done
		echo >> $Changelog;
	...
	echo -e ${grn}"\n √"${ylw}" Changelog successfully generated (file is $Changelog).\n"${txtrst}
fi
